...
- 🔄 **2026-09-19: sync สถานะทรัพย์ 62 หลังจากชีท 18-09 (Ben สั่ง — ยกเว้นกติกา insert-only เฉพาะคอลัมน์ `listing_status`)** · เพราะ insert-only ทำให้สถานะของเดิมค้าง: **51 หลังที่ขาย/ยกเลิกแล้วยังขึ้น Posted** · 4 หลังค้าง Ready to Post/Update (คิว Support หลอก) · 6 หลังว่างเพราะ `build_all_sql.py` (08-26) ใช้ชุดสถานะฮาร์ดโค้ดที่**ไม่มี `Need Info`/`Sold Completed`** → ทิ้งเป็น null · ก่อนเขียนเช็คแล้วว่าไม่มีใครเปลี่ยนสถานะผ่านเว็บเลย · update แบบมีเงื่อนไขค่าเดิม + audit_log 62 แถว (`changed_by` null, remark ระบุที่มา) · trigger เขียน support_log 62 แถว · หลัง sync ยอดตรงชีทเป๊ะ (Posted 319 · Sold Completed 102 · Cancel Completed 87 · Need Info 58 · ว่าง 2) · `v_support_listing` = 0
- 🔄 **2026-09-19: sync ลีด 189 รายจากชีท Active Lead 18-09 (Ben สั่ง)** — หา "ลีดที่ชีทเปลี่ยนหลังวันที่ถูก import" (เทียบกับไฟล์ชีทรุ่นที่ใช้ import: buyer_focus สำหรับรอบ 08-03 · Active Lead 08-26 สำหรับรอบ 08-26) ได้ 191 ราย 441 ช่อง แล้ว**เขียนทับเฉพาะช่องที่ค่าในฐานยังเท่ากับค่าตอน import** (ถ้าต่าง = มีคนแก้ในเว็บ → ข้าม) · 7 ช่อง: สถานะลีด 143 · Potential 99 · วันติดตามล่าสุด 74 · สเตจ 60 · คอมเมนต์ 58 · คอมมิชชั่น/วันปิด 1 · conflict 1 (L26-557 ต่างแค่ `\r\n`) · ก่อนเขียนเช็คแล้ว: ไม่มีเซลแก้ลีดผ่านเว็บ (audit มีแต่ของทดสอบ) · lead_stage_event 916 แถวเป็น baseline ทั้งหมด
  - ⚠️ **ปิด `trg_lead_stage_change` ระหว่าง sync** (ของ Ben P.) — ไม่งั้น 60 สเตจจะถูกนับเป็น "การย้ายสเตจวันนี้" ในแดชบอร์ด (`dash_stage_moves` นับ `not baseline`) → ใส่ event เองเป็น `baseline=true` · `at` = วันติดตามล่าสุด · audit_log 189 แถว · แจ้งเตือน 0 · หลัง sync: Active 137 · Reject 979 · Lose 33 · Win 32
  - 🔴 **ถ้า import/sync ลีดอีก ต้องปิด trigger 2 ตัว**: `trg_notify_lead_assigned` (แจ้งเตือน) + `trg_lead_stage_change` (นับสเตจ)
  - Stone ลบลีดทดสอบ 4 แถวเมื่อ 09-10 (`L26-007` · `LEAD-01` ×3) → เป็นเหตุที่ยอดลีดเคยเป็น 1,058 ไม่ใช่ 1,062
