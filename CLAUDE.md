...
> ⬜ ค้าง: Ben login เช็ค production แล้วปล่อยทีม · Ben Poovaviranon ต้อง `git pull` + แก้ `.mcp.json` ของตัวเอง · ไม่ได้ตั้ง `OPENAI_API_KEY` (Ben: ไม่เป็นไร) · ลบ `haus-migration.env` + `%TEMP%\haus-migration-dump` หลัง Ben ยืนยัน · เก็บโปรเจกต์เก่าไว้ 1–2 สัปดาห์ · `import/run_import.py` ยังชี้ฐานเก่า (ไม่เร่ง)
...
| 6 | **อัปเดตข้อมูลจากชีทอีกรอบ** — Ben แจ้งไว้ 2026-09-17 ว่าจะมี export ชุดใหม่มาอีก | ⬜ รอไฟล์จาก Ben |
