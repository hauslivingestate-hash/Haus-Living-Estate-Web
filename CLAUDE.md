...
### 🔴 ค้าง: `/contacts` — Ben ตั้งคำถามว่า **ตาราง `contacts` จำเป็นไหม** (ยังไม่ได้ข้อสรุป)
**อย่าเพิ่ง import อะไรลง `contacts` จนกว่าจะเคาะข้อนี้** — ตอนนี้ `contacts`/`contact_roles` ยัง **0 แถว** และหน้า `/contacts` ยังอ่าน seed

**ข้อมูลจริงที่ใช้ตัดสิน (นับจาก DB 2026-08-13):** เจ้าของ 452 (มีเบอร์ 401) · ลีด 953 (มีเบอร์ 891) · เบอร์ไม่ซ้ำรวม **1,169** · **ซ้อนกันแค่ 5 ราย (0.4%)** ที่เป็นทั้งเจ้าของและผู้ซื้อ
→ เหตุผลเดียวที่ตาราง unified มีอยู่คือ "คนคนเดียวมีหลายบทบาท" ซึ่งของจริงมี **5 ราย** · แลกกับการมีชื่อ/เบอร์ 2 ชุดที่ต้องคอยซิงก์ (ตาราง snapshot → เจ้าของ/ลีดที่สร้างใหม่หลัง import จะไม่โผล่เอง)

**ทางเลือกที่เสนอ Ben ไว้:** (ก) ทิ้งหน้า `/contacts` + ลบ 2 ตารางทิ้ง · (ข) เก็บหน้าไว้แต่ทำเป็น **view รวมสดๆ** จาก `main_2_owner` + `main_6_buyer_crm` (ไม่มีตารางใหม่ ไม่มี snapshot drift)
