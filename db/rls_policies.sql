...
-- ============================================================
-- 15) 2026-08-17 — แจ้งเตือนตอนมอบหมายลีด (trigger, ไม่ใช่ RPC)
-- ============================================================
-- policy INSERT ของ `notifications` เป็น own-row โดยตั้งใจ (กันคนยิงแจ้งเตือนปลอมใส่กัน)
-- ผลข้างเคียง: คนที่มอบลีดให้คนอื่นเขียนแจ้งเตือนถึงคนนั้นไม่ได้ — ซึ่งเป็นจุดประสงค์
-- ทั้งหมดของแจ้งเตือนตัวนี้ · ตอนนี้มอบหมายได้ 3 คน: Stone (ceo) + Admin (system_admin)
-- ผ่านอยู่แล้วเพราะมี roles.manage · **Benz (listing_support) คือคนเดียวที่ติด**
--
-- เลือก trigger แทน RPC เพราะ:
--   1) ลีดเข้าทาง n8n เป็นหลัก (952/953 มี sale_id มาแต่ต้นทาง) RPC จะครอบเฉพาะตอนกดปุ่มในเว็บ
--   2) ไม่ต้องขยายสิทธิ์ให้ใครเลย
--   3) ปลอมไม่ได้ — ข้อความระบบเขียนเอง ไม่รับจากผู้ใช้
--
-- 4 ด่านในฟังก์ชัน (ทดสอบครบแล้ว): อัปเดตที่ไม่ได้เปลี่ยนเจ้าของ → ไม่เตือน ·
-- มอบให้ตัวเอง → ไม่เตือน · ผู้รับต้อง Active · ไม่มี session (n8n) → actor = 'ระบบ'
--
-- ⚠️ IMPORT ลีดเป็นก้อนอีกครั้งเมื่อไหร่ ให้ปิด trigger ก่อน ไม่งั้นยิงแจ้งเตือนพันกว่าใบ:
--     alter table main_6_buyer_crm disable trigger trg_notify_lead_assigned;
--
-- (นิยามเต็มอยู่ใน migration `notify_on_lead_assignment` — ดู supabase_full_setup.sql)

-- ============================================================
-- 16) 2026-08-23 — แจ้งเตือนตามเวลา (pg_cron)
-- ============================================================
-- งานที่ไม่มีใคร "กด" อะไรตอนที่มันควรเตือน: ลีดค้าง · ใบลารออนุมัติ ·
-- ดีลปิดแล้วยังไม่กรอกราคา · ทรัพย์ที่ประกาศอยู่แต่ยังไม่มีรูป
--
-- ใช้ `pg_cron` ในตัว Supabase — ไม่ต้องมีเซิร์ฟเวอร์แยก ไม่ต้องตั้ง Vercel cron
-- job: `daily-notifications` เวลา 02:00 UTC = 09:00 ไทย → select run_daily_notifications()
--
-- ⚠️ 3 เรื่องที่ตัดสินใจไว้ (Ben, 2026-08-23) และเปลี่ยนไม่ได้ตามใจ:
--   1) **ใบสรุปวันละใบต่อคน ไม่ใช่ใบต่อรายการ** — ลีดค้างมี ~925 ราย ถ้ายิงใบต่อลีด
--      กระดิ่งจะกลายเป็นขยะวันแรกและไม่มีใครเปิดดูอีก
--   2) **นาฬิกาลีดค้างเริ่มนับที่ `notification_cron_state.started_at`** ไม่ใช่
--      `last_follow_date` — ช่องนั้นมาจากชีทตอน import (ลีด 337 รายไม่เคยมีค่าเลย)
--      → วันเปิดระบบได้ 0 ใบ แล้วค่อยเริ่มเตือนหลังพ้น 7 วัน ซึ่งตอนนั้นค้างจริงแล้ว
--   3) **รูป: เฉพาะสถานะที่ยังมีชีวิต** (Posted / Ready to Post / Update) —
--      Sold Completed / Cancel Completed ไม่ต้องตามรูปแล้ว
--
-- ⚠️ ฟังก์ชันเป็น `security definer` เพราะเขียนแจ้งเตือนถึงคนอื่น ซึ่ง policy own-row
--    ของ `notifications` ปฏิเสธโดยตั้งใจ (เหตุผลเดียวกับ trigger มอบหมายลีด §15)
-- ⚠️ กันยิงซ้ำด้วยการเช็คว่าวันนี้คนนั้นได้ชนิดนั้นไปแล้วหรือยัง — รัน cron ซ้ำได้ปลอดภัย
-- ⚠️ **`notifications.type` มี CHECK constraint** เพิ่มชนิดใหม่ต้องแก้ constraint ด้วย
--    ไม่ใช่แค่แก้ union ฝั่ง TypeScript (รันครั้งแรกล้มเพราะข้อนี้)
--
-- (นิยามเต็มอยู่ใน migration `daily_reminder_notifications` + `notification_types_for_reminders`)

-- ============================================================
-- 17) 2026-08-23 — เทมเพลตคำโฆษณา + เทมเพลต KPI
--     (migration `copy_and_kpi_templates` + `kpi_template_identity_by_default`)
-- ============================================================
-- 2 ใน 3 หน้าตั้งค่าที่เคยแก้ลง React state แล้วรีเฟรชหาย · เหลือ "เช็คลิสต์ทรัพย์" อีกหน้าเดียว
--
-- `listing_copy_template` (PK grade+copy_type) — เขียนได้ด้วย copy.manage
--   ⚠️ **เก็บเฉพาะช่องที่มีคนแก้ ไม่ได้เก็บครบ 9 ช่อง** ค่าตั้งต้นอยู่ในโค้ด
--      (lib/listingCopy.ts defaultTemplate) และเป็น fallback เสมอ
--      → ตารางว่าง = คำโฆษณายังออกถูกทุกช่อง · "คืนค่าเริ่มต้น" = ลบแถว ไม่ใช่เขียนค่าตั้งต้นทับ
--      → ช่องที่ไม่มีใครแก้จะได้อานิสงส์เวลาปรับค่าตั้งต้นในโค้ด
--   grade/copy_type ใช้ check ไม่ใช่ FK เพราะเป็นค่าที่ normalize แล้ว
--      ("A List + Fb add" กับ "A List" ยุบเป็น a_list ช่องเดียวกัน)
--
-- `kpi_template` — เขียนได้ด้วย masterdata.govern · seed 6 แถวตรงกับ KPI_TEMPLATES เดิมในโค้ด
--   `activity_type` FK → action_type (on update cascade) เปลี่ยนชื่อกิจกรรมแล้วเทมเพลตตามไปเอง
--   check `kpi_template_activity_coherent`: source='activity' ต้องมีกิจกรรม · แบบอื่นต้องไม่มี
--   🔴 **id ต้องเป็น `generated BY DEFAULT as identity` ห้ามเป็น `always`** —
--      upsert คือ INSERT ... ON CONFLICT ที่ต้องส่งคอลัมน์ id มาด้วยเพื่อหา conflict
--      `always` ปฏิเสธค่า id ที่ส่งมาทุกกรณี → บันทึกแถวเดิมล้มเงียบทุกครั้ง (เจอจริงตอนทดสอบ)
--
-- ทั้ง 2 ตาราง: select ได้ทุกคนที่ล็อกอิน (ป้อนปุ่มสร้างคำโฆษณา / หน้าตั้งเป้า)
-- ทดสอบแล้ว: agent เขียนไม่ได้ทั้งคู่ (42501) แต่อ่านได้ · marketing เขียนคำโฆษณาได้ KPI ไม่ได้ ·
--            check ปฏิเสธ 23514 · FK ปฏิเสธกิจกรรมที่ไม่มีจริง 23503

-- ============================================================
-- 18) 2026-08-23 — เช็คลิสต์ทรัพย์เด่น (migration `listing_checklists`
--     + `exclusive_agreement_dates` + `v_main_listing_agreement_dates`)
-- ============================================================
-- ปิดครบทั้ง 3 หน้าตั้งค่าที่เคยแก้ลง React state แล้วรีเฟรชหาย
--
-- `checklist_template` + `checklist_template_item` — นิยาม แก้ได้ด้วย checklists.manage
-- `listing_checklist_item` (PK listing_id+template_item_id) — ความคืบหน้าต่อทรัพย์
--
-- ⚠️ **สิทธิ์ 2 ชั้นต่างกันโดยตั้งใจ**: แก้ "ว่าขั้นตอนคืออะไร" ต้อง checklists.manage
--    แต่ "ติ๊ก" ใช้สิทธิ์เท่ากับแก้ทรัพย์ (listings.edit / listings.marketing)
--    เพราะงานเช็คลิสต์เป็นงานข้ามทีม — เซลคุยเจ้าของ · Support เก็บเอกสาร · การตลาดโพสต์
--    ถ้าผูกกับเซลเจ้าของทรัพย์คนเดียว ส่วนใหญ่จะติ๊กไม่ได้เลย
--
-- ⚠️ **ความคืบหน้าเป็น live join กับเทมเพลต ไม่ใช่ snapshot** — แถวเกิดตอนมีคนแตะครั้งแรก
--    เท่านั้น → เพิ่มขั้นตอนใหม่ ทรัพย์ทุกหลังเห็นทันทีโดยไม่ต้อง backfill
--    และ **ลบขั้นตอน = ความคืบหน้าของขั้นตอนนั้นในทุกทรัพย์หายตาม (cascade)** ซึ่งถูกต้อง
--    (ขั้นตอนที่ไม่มีแล้วจะทำครึ่งๆ ไม่ได้) แต่เป็นเหตุผลที่หน้าจอถามก่อนลบ
--
-- ⚠️ **เอกสารเก็บ "ลิงก์" ไม่ใช่ไฟล์** (Ben ตัดสินใจ) — โฉนด/สำเนาบัตร ปชช. อยู่ใน Google Drive
--    ตามเดิม ไม่เอา PII เข้า Supabase และไม่กินโควตาที่รูปทรัพย์ใช้อยู่
--
-- ⚠️ **`agreement_start`/`agreement_end` อยู่บน `main_4` ไม่ใช่ `main_10`** —
--    trigger `sync_potential_listing` ลบแถว main_10 ทิ้งทันทีที่ทรัพย์หลุดเกณฑ์
--    วันหมดสัญญาที่เซ็นไว้จะหายไปด้วย · เพิ่มเข้า `v_main_listing` แล้ว (แอปอ่านผ่าน view เท่านั้น)
--
-- 🔴 **เลิกใช้ 5 คอลัมน์ใน `main_10_potential_listing`** (template_link · marketplace ·
--    profile · group_date · group_boost_date) — ซ้ำกับเทมเพลต "ลงประกาศ A List" เป๊ะทั้ง 7 ข้อ
--    ตอนเลิกใช้ทั้ง 5 ยังว่าง 0/210 จึงไม่มีข้อมูลหาย · main_10 ยังทำหน้าที่เดิมคือ
--    "ทรัพย์ไหนเป็น A List" ผ่าน trigger ไม่ได้ถูกแตะ
--
-- ทดสอบแล้ว: agent เห็นเทมเพลต 3 อัน แก้ไม่ได้ (0 rows) แต่ติ๊กได้ (ถูกต้อง) ·
--            ลบเทมเพลต → ขั้นตอน + ความคืบหน้าหายตาม cascade ·
--            check ปฏิเสธ 23514 ทั้ง 3 กรณี (task ที่ถือ repeat_days · cadence ที่ไม่มีรอบ ·
--            เทมเพลตที่เล็ง Normal)
